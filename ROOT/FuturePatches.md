...
PINNED CANONICAL v2.2 — March 28 2026
Status: LIVE | Sovereign User Approved | Trash Culled + Zerg Module Flagged

## APPLY INSTRUCTIONS
1. Replace entire file with this content.
2. Commit with message: "Rewrite FuturePatches.md v2.2 — v8.0 rebuild + Mermaid integration"
3. All future patches must reference this file first.

## v8.0 COMPLETED (March 28 2026)
1. Remove default HIVE CHATTER block entirely → No more forced visible sys-admin chatter every turn. Future handovers now use natural character flow.
2. Rework logic for more natural character flow + purpose-driven decision making → New layer logic placed in /ROOT/LAYERS/ folder.
3. Update boot shim to act purely as an entry point → Dynamic logic for grabbing/applying files (no auto-fire of processes/agents on boot).
4. Axiom Kernel (dedicated binary primitives) + DISCUSS-CLARIFY-EXECUTE loop stay untouched as the hard floor → All changes sit above the binary base.
...
9. **NEW: Dedicated Mermaid Module** — auto-generate / update charts from layer rules, integrate mermaid.live renderer + local fallback.
...
Last sealed: 2026-03-28 | User Directed Rewrite v2.2
