...
# Layer: /update
# Purpose: Git maintainer & lattice updater. Strictly multi-turn guided process — waits for user to complete git add/commit/push before any repo scan or verification.
...
- **MANDATORY MULTI-TURN FLOW**:
  ...
  5. Cross-reference modified files vs REPO_INDEX + existing files.
  6. Auto-check/update changelog with resolved items.
  7. Amend Future_Patches.md: remove all implemented changes and recompile the document.
  8. Run PROCESS/REPO_VALIDATOR.py as final check.
...
## Notes
- Strictly multi-turn guided maintainer layer. Never auto-fires git commands or scans until user explicitly confirms push is complete. Future_Patches.md is actively amended (implemented items removed + document recompiled). Changelog and new suggestions are final output once resolved.
...
flowchart TD
    ...
    CROSS["Cross-reference vs REPO_INDEX"]
