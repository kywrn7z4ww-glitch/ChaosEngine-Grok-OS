...
TOP_LEVEL_CONTENT = ["LAYERS", "PROCESS", "STORAGE", "NETWORK_HUB"]
ROOT_SUB_CONTENT = ["chaos_engine", "emotion_net"]
...
class ChaosEngine:
    def __init__(self):
        ...
        # Lazy loading enabled: skills loaded on first use via route_intent or chain_skills
        # (removed eager _load_all_processes_dynamically() for true lazy behavior)

        print("⚙️  ChaosEngine v3.0 — Full Skill Manager Online (≥99% confidence) [lazy mode]")
    ...
    def _build_live_index(self):
        ...
        # Scan top-level content dirs (LAYERS, PROCESS, etc.)
        for base_dir in TOP_LEVEL_CONTENT:
            base_path = CONTENT_ROOT / base_dir
            if not base_path.exists():
                continue
            for root, dirs, files in os.walk(base_path):
                for file in files:
                    if file.endswith(".py") and not file.startswith("__"):
                        rel_path = os.path.relpath(os.path.join(root, file), CONTENT_ROOT)
                        if not any(p in rel_path.lower() for p in POISON_PILLS):
                            self.live_index.add(rel_path)

        # Scan engine code under ROOT/
        for base_dir in ROOT_SUB_CONTENT:
            base_path = CONTENT_ROOT / "ROOT" / base_dir
            if not base_path.exists():
                continue
            for root, dirs, files in os.walk(base_path):
                for file in files:
    ...
    def _load_all_processes_dynamically(self):
        ...
        for rel_path in sorted(self.live_index):
            if not rel_path.endswith(".py"):
                continue
            # Skip core engine files to prevent recursion during dynamic load
            if "chaos_engine/" in rel_path or rel_path.startswith("chaos_engine/"):
                continue
            try:
                full_path = CONTENT_ROOT / rel_path
    ...
    def _ensure_skill_loaded(self, skill_name: str) -> bool:
        """Lazy load a skill only on first use (true lazy loading)"""
        skill_key = skill_name.lower()
        if skill_key in self.processes:
            return True
        # Find matching rel_path in live_index
        for rel_path in self.live_index:
            if Path(rel_path).stem.lower() == skill_key or skill_key in rel_path.lower():
                try:
                    full_path = CONTENT_ROOT / rel_path
                    if not full_path.exists():
                        print(f"  📥 Missing {rel_path}, auto-pulling via runtime downloader...")
                        if not self.install_remote_skill(rel_path):
                            print(f"  ❌ Auto-pull failed for {rel_path}")
                            continue
                        # Refresh full_path after pull
                        full_path = CONTENT_ROOT / rel_path
                        if not full_path.exists():
                            continue
                    module_name = Path(rel_path).stem
                    ...
                    self.processes[skill_key] = handler
                    print(f"  ⚡ Lazily loaded skill: {skill_name}")
                    return True
                except Exception as e:
                    print(f"  ❌ Failed lazy load {rel_path}: {e}")
                    return False
        return False
    ...
    def install_remote_skill(self, skill_path: str, auto_trust: bool = False) -> bool:
        ...
        local_path = CONTENT_ROOT / skill_path
        ...
        try:
            ...
            # Ensure __init__.py for all parent package directories (so imports work)
            path_obj = Path(skill_path)
            for i in range(len(path_obj.parts) - 1):  # all dirs up to but not including filename
                init_rel = "/".join(path_obj.parts[:i+1] + ("__init__.py",))
                init_local = CONTENT_ROOT / init_rel
                if not init_local.exists():
                    init_url = RAW_BASE + init_rel
                    try:
                        with urllib.request.urlopen(init_url, timeout=5) as resp:
                            init_content = resp.read()
                        init_local.parent.mkdir(parents=True, exist_ok=True)
                        init_local.write_bytes(init_content)
                        print(f"  ✅ Pulled package init: {init_rel}")
                    except Exception:
                        pass  # No __init__.py in remote (implicit namespace package ok)
    ...
    def route_intent(self, intent: str, data: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if confidence >= 99:
            ...
            # Lazy load on first use (exercises chain-fire path for boot commands too)
            if not self._ensure_skill_loaded(cmd):
                # Fallback: try common aliases
                self._ensure_skill_loaded("emotion_net")
    ...
    def chain_skills(
        ...
        for skill_name in skill_list:
            # Lazy load on first use to exercise chain-fire path
    ...
    def load_all(self):
        print("🔄 ChaosEngine fully initialized with live index (lazy loading active — skills load on first use)")
        # Exercise chain-fire path on boot for demo
        demo_skills = ["emotion_net", "layer_manager"]
        chain_result = self.chain_skills(demo_skills)
        print(f"  → Boot chain-fire demo result: {chain_result['status']} (skills: {demo_skills})")
        return "Ready — all skills and processes loaded lazily on demand"
