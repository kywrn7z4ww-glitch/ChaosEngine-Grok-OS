...
# v4.2 – quick-by-default + escalate on demand / repeat calls / code audit hints
# + external anchors: Grokipedia (pri 1) → Wikipedia (pri 2) → Perplexity (pri 3)
# Stays tiny & chill unless deliberately escalated or low conf/surprise detected
# External calls throttled: max 1 per check, only on factual drift risk
...
class TruthChecker:
    ...
    def check(self, output: str, escalate: bool = False, anchor_pri: str = "grok") -> str:
        self.check_count += 1
        ...
        # ────────────────────────────────
        # EXTERNAL ANCHOR CASCADE (only if needed)
        # ────────────────────────────────
        external_delta = 0
        if (score < 82 or len(self.unverified) > 1 or self.lat.get('surprise', 0) > 0.32 or conf < 0.68 or escalate):
            topic = self._extract_topic(output)
            external_delta = self._anchor_external(output, topic, anchor_pri)
            lines.append(external_delta)
            score = max(0, min(100, score + external_delta))  # apply delta to final score
    ...
    def _extract_topic(self, text: str) -> str:
        # Crude but effective topic guess from first factual sentence
    ...
    def _anchor_external(self, claim: str, topic: str, pri: str = "grok") -> str:
        anchors = {}
        delta = 0

        # Priority 1 – Grokipedia (xAI aligned)
        if pri in ("grok", "all"):
            url = f"https://grokipedia.com/page/{topic}"
            summary = self._harvest(url, "Extract key facts, dates, entities only. Neutral summary, no fluff.")
            anchors['grok'] = summary
            match_score = self._fuzzy_match(claim, summary)
            if match_score > 0.75:
                delta += 14
            elif self._opposing(claim, summary):
                delta -= 20

        # Priority 2 – Wikipedia (neutral baseline)
        if pri in ("wiki", "all") or delta < 0:
            url = f"https://en.wikipedia.org/wiki/{topic}"
            summary = self._harvest(url, "Pull lead paragraph + key facts. Neutral summary.")
            anchors['wiki'] = summary
            match_score = self._fuzzy_match(claim, summary)
            if match_score > 0.7:
                delta += 10
            elif self._opposing(claim, summary):
        ...
        # Priority 3 – Perplexity (real-time + citations)
        if pri == "perp" or (delta < 5 and pri != "grok"):
            url = f"https://www.perplexity.ai/search?q={claim.replace(' ', '+')}"
            summary = self._harvest(url, "Extract concise answer summary + top 3 cited sources only. No ads or UI junk.")
            anchors['perp'] = summary
            match_score = self._fuzzy_match(claim, summary)
            if match_score > 0.7:
                delta += 8

        self.lat['anchors'] = anchors
        sources = list(anchors.keys())
        return f"External anchor delta: {delta:+.0f} conf | sources: {', '.join(sources) or 'none'}"

    def _harvest(self, url: str, instructions: str) -> str:
        # In live Grok OS: this calls browse_page(url, instructions)
        # Placeholder for repo / dev — real fetch happens at runtime via CannonHarvester or direct tool
        return f"[HARVESTED SUMMARY FROM {url} — instructions: {instructions}]"
